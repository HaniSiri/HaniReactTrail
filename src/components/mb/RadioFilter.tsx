...
const RadioFilter = ({ filterData }: RadioProps) => {
                ...
                {filterData.values.map((val, index) => {
                    return (
                        <div className={`min-h-12 flex items-center ${!showAll && index > 4 ? 'hidden' : ''}`}>
                            <input type="radio" checked={val.selected} className='custom-radio' id={filterData.name} name={filterData.name} value={val.value}></input>
                            {val.image && (<img className='h-4 pl-2' src={val.image} alt={val.name}></img>)}
